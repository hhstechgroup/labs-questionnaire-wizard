...
public class ComponentValidator implements Validator {
    ...
    public ComponentValidator(WizardQuestion question, int pageNumber,
	    int topicNumber, boolean isParent) {
	...
	case PARAGRAPHTEXT:
	    if (question.isRequired()
		    && !validateTextAreaQuestionComponent(value)) {
	...
	case CHOOSEFROMLIST:
	    if (question.isRequired()
		    && !validateDropDownQuestionComponent(value)) {
		question.setValid(false);
		throw new ValidatorException(new FacesMessage(
			FacesMessage.SEVERITY_ERROR, "Validation Error",
			"Answer must be selected for this question!"));
	...
	case FILEUPLOAD:
	    ...
	    OutputLabel outputLabel = (OutputLabel) FacesContext
		    .getCurrentInstance().getViewRoot()
		    .findComponent("maincontentid-little_" + question.getId());
