...
public class WizardDataModelGenerator {
	...
	for (QuestionnaireForms questionnaireForms : forms) {
		...
		wizardForm
			.setWizardPageList(getWizardPages(questionnaireForm.getPages()));
	...
	case CHOOSEFROMLIST:
		...
		if (!xmlQuestion.getDefaultAnswers().getDefaultAnswer().isEmpty()) {
		    ...
		    chooseFromListDefaults.setValue(xmlQuestion.getDefaultAnswers()
	...
	case MULTIPLECHOICE:
		...
		if (!xmlQuestion.getDefaultAnswers().getDefaultAnswer().isEmpty()) {
		    ...
		    multipleChoiceDefaults.setValue(xmlQuestion.getDefaultAnswers()
			    .getDefaultAnswer().get(0));
	...
	case RANGE:
	    ...
	    rangeQuestion.setRange(xmlQuestion.getRange().getRangeBegin(), xmlQuestion
