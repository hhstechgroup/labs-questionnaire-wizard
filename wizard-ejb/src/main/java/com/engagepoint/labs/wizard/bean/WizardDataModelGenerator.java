...
public class WizardDataModelGenerator {
    ...
    private static final Logger LOGGER = Logger
            .getLogger(WizardDataModelGenerator.class.getName());
    ...
    private void getTimeDefaultAnswer(Question xmlQuestion,
        ...
        if (checkDefaultAnswer(xmlQuestion)) {
            ...
            try {
                ...
                LOGGER.warn("TimeParseExceptionOne", e);
            }
            timeDefault.setValue(time);
            wizardQuestion.setDefaultAnswer(timeDefault);
            wizardQuestion.setAnswer(timeDefault);
        } else {
            DateValue timeDefault = new DateValue();
            SimpleDateFormat formatter = new SimpleDateFormat(
                    TimeQuestion.TIME_FORMAT);
            try {
                time = formatter.parse("12:00");
            } catch (ParseException e) {
                LOGGER.warn("TimeParseExceptionTwo", e);
            }
            timeDefault.setValue(time);
            wizardQuestion.setDefaultAnswer(timeDefault);
            wizardQuestion.setAnswer(timeDefault);
    ...
    private void getDateDefaultAnswer(Question xmlQuestion,
                                      WizardQuestion wizardQuestion) {
        Date date = null;
        if (checkDefaultAnswer(xmlQuestion)) {
    ...
    private void getTextDefaultAnswer(Question xmlQuestion,
                                      WizardQuestion wizardQuestion) {
        if (checkDefaultAnswer(xmlQuestion)) {
    ...
    private void getParagraphText(Question xmlQuestion,
                                  WizardQuestion wizardQuestion) {
        if (checkDefaultAnswer(xmlQuestion)) {
    ...
    private MultipleChoiseQuestion getMultipleChoiseQuestion(
            Question xmlQuestion) {
    ...
    private boolean checkDefaultAnswer(Question xmlQuestion) {
        if (xmlQuestion.getDefaultAnswers() != null
                ...
                .isEmpty()) {
            List<String> defaultAnswersList = getAllCorrectDefaultAnswers(xmlQuestion
                    .getDefaultAnswers());
            if (defaultAnswersList.isEmpty()) {
                return false;
            }
            this.defaultAnswers = defaultAnswersList;
            return true;
        }
        return false;
    }
    ...
    private boolean checkQuestionRules(Question xmlQuestion) {
        ...
        List<QuestionRule> questionRules = null;
        if (rules != null) {
            questionRules = rules.getQuestionRule();
        }
        if (questionRules != null && !questionRules.isEmpty()) {
    ...
    private boolean checkGroupRules(Group xmlGroup) {
        ...
        List<GroupRule> groupRules = null;
        if (rules != null) {
            groupRules = rules.getGroupRule();
        }
        if (groupRules != null && !groupRules.isEmpty()) {
    ...
    private boolean checkPageRules(Page xmlPage) {
        ...
        List<PageRule> pageRules = null;
        if (rules != null) {
            pageRules = rules.getPageRule();
        }
